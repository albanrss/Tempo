...
class _ActiveLimitsScreenState extends State<ActiveLimitsScreen> {
  ...
  Future<Map<String, int>> _getUsageToday(Set<String> packages) async {
    ...
    try {
      ...
      for (final entry in lastForeground.entries) {
        usageMap[entry.key] =
            (usageMap[entry.key] ?? 0) + (nowMs - entry.value);
      }
    } catch (_) {}
  ...
  Future<void> _loadLimits() async {
    ...
    final limitKeys =
        keys.where((k) => k.startsWith(StorageKeys.timeLimitPrefix));
    ...
    for (final packageName in packageNames) {
      ...
      final appName = cachedApp?.name ??
          prefs.getString('${StorageKeys.appNamePrefix}$packageName') ??
          packageName;

      entries.add(_LimitedAppEntry(
        name: appName,
              ...
              Expanded(
                child: RefreshIndicator(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final remaining = (entry.minutes - entry.usedMinutes)
                        .clamp(0, entry.minutes);
                    final isExhausted = remaining <= 0;
                    return ListTile(
                      contentPadding:
                          const EdgeInsets.fromLTRB(16, 8, 16, 8),
                      leading: entry.icon != null
                          ? Image.memory(entry.icon!,
